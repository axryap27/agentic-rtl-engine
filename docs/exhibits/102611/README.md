...
are committed unchanged so the milestone is inspectable without re-running a metered live run.

> **The claim this exhibit backs:** the LLM never wrote the multiplier. It wrote `product = a * b`
> and proposed a loop invariant. The shift-add datapath was *derived*, installed only after the
> obligation kernel proved it correct over **all 65,536 input pairs**.
...
| 3 | `02_formal_spec.json` | **Stage 3 (Agent 3): the abstract spec.** One transition, `product = a * b`, `spec_statement: true`. This is the hand-authored design in full: no datapath, no FSM, no registers. |
...
kernel ran an **exhaustive finite proof** over every pair (no sampling). Had the invariant been
wrong, this rule would have been a pure no-op (the engine's backtrack signal), with a concrete
counterexample.

**Step 1: `ScheduleHandshakeFSM`.** The verified loop is mechanically scheduled onto the
hardened IDLE(0)/BUSY(1)/DONE(2) control FSM with a `start`/`done` handshake. Soundness lives in
step 0; this step is deterministic scheduling, no proof needed.
...
The run passed (`status: success`) but flagged 19 vector disagreements. The cross-check works as
designed: in every case Agent 1 asserted `product = 0` during the multi-cycle BUSY phase, where the
spec correctly shows the accumulating partial products. Both sides agree at every `DONE` and on
`done` everywhere. The guardrail records every disagreement so a passing run stays visible when
Agent 1 and the spec differ.
