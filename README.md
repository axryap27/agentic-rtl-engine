...
An LLM asked to emit RTL in one shot hallucinates: plausible code that breaks in hardware. So the LLM never freehand-writes hardware here. At each step it picks one move from a short, fixed menu of correctness-preserving transformations, the bounded action space. Constraining the choices starves the failure mode.
...
- A formal spec is **abstract** (what should happen); hardware is **concrete** (clocks, registers, wires). Asking an LLM to rewrite an abstract spec as RTL in one shot is the kind of open-ended request that invites hallucination.
- The engine uses **stepwise refinement** instead: it lowers the spec toward hardware one transformation at a time, each a single rule from a small, fixed library. Every rule is provably refinement-preserving, so a design built only from library rules is correct by construction.
- Per step, the LLM's only job is to choose the next applicable rule and return a `(rule, parameters)` pair.
- For sequential arithmetic, **verified derivation** goes further: the LLM authors only an abstract postcondition (e.g. `product = a * b`) plus a candidate loop invariant. A deterministic proof kernel (`pipeline/refinement/obligations.py`) discharges the Morgan/Back refinement obligations against the real expression semantics before the loop is installed: an exhaustive proof over the declared bit widths when the input space is ≤ 65,536, sampled falsification otherwise, with a concrete counterexample on failure. The LLM proposes; the kernel proves.
- Every applied rule is logged to `refinement_chain.json`: a replayable proof trail from abstract spec to RTL. Verified derivations carry their discharged proof audit (invariant, variant, guard, mode, cases checked) as a certificate on the chain entry.

---

## Architecture: four stages

**LangGraph** orchestrates the run, advancing, retrying, or halting on the `status` field each stage writes to disk. Stages pass data as JSON in the run's artifact directory (`artifacts/<date>/<time-module-hash>/`), so every run is inspectable.
...
- **cocotb**: a Python-based HDL verification framework. Testbench stimulus comes from the Stage 1 vectors; expected outputs are re-derived from the refined spec by an independent interpreter (`pipeline/cocotb/spec_sim.py` + `vector_check.py`), so a Stage 1 arithmetic slip can't fail a correct design. Agent-1/spec disagreements are recorded and surfaced.
...
The LLM is confined to four jobs: interpreting the prompt, authoring/revising the formal spec, picking refinement rules, and classifying simulation failures. Stage 2, both compilers, the refinement engine and rules, the proof kernel, the vector checker, the soak, and the cocotb runner are deterministic Python with no LLM calls.
...
Agent 3 is a distinct, tool-using Anthropic agent with its own credential. Without that key, Stages 1 and 2 run but Stage 3 halts with a "key not configured" error. This Anthropic key is billed per token, separate from any Claude subscription (which does not cover direct API usage).
...
Runs the full pipeline on the default 2-bit counter. Output lands in a dated, module-named run dir (`artifacts/<date>/<time-module-hash>/`), with `artifacts/latest` pointing at the newest run; every intermediate artifact is on disk for inspection. Prune old runs with `python3.11 main.py --clean-artifacts [N]`.

---

## Tests

The suite is fully deterministic, no LLM calls: 469 tests, ~30 s. Fast, free, safe to run anytime:

```bash
python3.11 -m pytest tests/ -q
```

It exercises the mechanical path end to end (engine, bridge, both compilers, cocotb) with hand-built specs and a scripted rule-picker standing in for the LLM. Coverage:
...
The pipeline runs live end to end on all seven design classes. The verified-derivation run is the hardest test of the bounded-action-space claim: the LLM proposes the abstract spec and invariant, the kernel proves the derived shift-add loop over all 65,536 input pairs before installing it, the scheduler lowers it onto a start/done handshake FSM, and the result passes cocotb and a clean random soak.

The artifacts from run `102611` are committed verbatim as an inspectable exhibit: **[`docs/exhibits/102611/`](docs/exhibits/102611/README.md)**, including the hand-authored abstract spec (`02_formal_spec.json`) and the derivation certificate (`refinement_chain.json`) with its discharged invariant and variant. The same mechanism is reproducible offline, no credentials needed, via `pytest tests/test_verified_derivation.py`.
